...
const char* output_suffix(const QuantizationOptions& options) {
    if (options.u_transpose) {
        if (options.bits == 4) return ".eigen.u4utc.bin";
        if (options.bits == 8 && options.entropy_coding) return ".eigen.u8eutc.bin";
        if (options.bits == 8) return ".eigen.u8utc.bin";
        if (options.bits == 16) return ".eigen.u16utc.bin";
    }
...
/** U-transpose runtime layout: one scale per eigen column; matrix stored row-major by SNP row. */
static void quantize_file_u_transpose(const fs::path& input_dir,
                                      const fs::path& output_dir,
                                      const string& name,
                                      const QuantizationOptions& options,
                                      QuantizationSummary& summary) {
    ...
    try {
        ...
        // Input U payload is column-major: each eigenvector column j has m values.
        vector<float> u(static_cast<size_t>(m) * static_cast<size_t>(k));
        for (int32_t j = 0; j < k; ++j) {
            if (fread(u.data() + static_cast<size_t>(j) * static_cast<size_t>(m), sizeof(float),
                      static_cast<size_t>(m), fp) != static_cast<size_t>(m)) {
        ...
        vector<float> scales_eig(k, 0.0f);
        for (int32_t j = 0; j < k; ++j) {
            float s = 0.0f;
            const size_t off = static_cast<size_t>(j) * static_cast<size_t>(m);
            for (int32_t i = 0; i < m; ++i) {
                s = max(s, fabsf(u[off + static_cast<size_t>(i)]));
            }
            scales_eig[j] = s;
        }
        ...
        if (fwrite(scales_eig.data(), sizeof(float), k, out) != static_cast<size_t>(k)) {
            throw runtime_error("Write error (eigen scales) in " + output_path.string());
        }

        const bool q8_entropy = (options.bits == 8 && options.entropy_coding);
        vector<int8_t> matrix_q8;
        if (q8_entropy) {
            matrix_q8.resize(static_cast<size_t>(m) * static_cast<size_t>(k));
        }
        ...
        for (int32_t i = 0; i < m; ++i) {
            if (options.bits == 4) {
                ...
                vector<uint8_t> packed(packed_k);
                for (int32_t j = 0; j < k; j += 2) {
                    const float v0 = u[static_cast<size_t>(j) * static_cast<size_t>(m) + static_cast<size_t>(i)];
                    const float v1 = (j + 1 < k)
                        ? u[static_cast<size_t>(j + 1) * static_cast<size_t>(m) + static_cast<size_t>(i)]
                        : 0.0f;
                    const int16_t q0 = quantize_value(v0, scales_eig[j], options.bits);
                    const int16_t q1 = quantize_value(v1, (j + 1 < k) ? scales_eig[j + 1] : 1.0f, options.bits);
                    packed[static_cast<size_t>(j / 2)] =
                        static_cast<uint8_t>((q0 & 0x0F) | ((q1 & 0x0F) << 4));
                }
                if (fwrite(packed.data(), sizeof(uint8_t), packed_k, out) != static_cast<size_t>(packed_k)) {
                    throw runtime_error("Write error (quantized U row) in " + output_path.string());
                }
            } else if (options.bits == 8) {
                if (q8_entropy) {
                    for (int32_t j = 0; j < k; ++j) {
                        const float v = u[static_cast<size_t>(j) * static_cast<size_t>(m) + static_cast<size_t>(i)];
                        matrix_q8[static_cast<size_t>(i) * static_cast<size_t>(k) + static_cast<size_t>(j)] =
                            static_cast<int8_t>(quantize_value(v, scales_eig[j], options.bits));
                    }
                } else {
                    vector<int8_t> row_q(k);
                    for (int32_t j = 0; j < k; ++j) {
                        const float v = u[static_cast<size_t>(j) * static_cast<size_t>(m) + static_cast<size_t>(i)];
                        row_q[static_cast<size_t>(j)] =
                            static_cast<int8_t>(quantize_value(v, scales_eig[j], options.bits));
                    }
                    if (fwrite(row_q.data(), sizeof(int8_t), k, out) != static_cast<size_t>(k)) {
                        throw runtime_error("Write error (quantized U row) in " + output_path.string());
                    }
                }
            } else {
                vector<int16_t> row_q(k);
                for (int32_t j = 0; j < k; ++j) {
                    const float v = u[static_cast<size_t>(j) * static_cast<size_t>(m) + static_cast<size_t>(i)];
                    row_q[static_cast<size_t>(j)] = quantize_value(v, scales_eig[j], options.bits);
                }
                if (fwrite(row_q.data(), sizeof(int16_t), k, out) != static_cast<size_t>(k)) {
                    throw runtime_error("Write error (quantized U row) in " + output_path.string());
                }
            }
        }
        ...
        cout << "Quantized (U transpose) " << name << ".eigen.bin"
             << " -> " << name << output_suffix(options) << " (m=" << m << ", k=" << k
             << ", q=" << options.bits << (options.entropy_coding ? ", zlib" : "") << ", bytes "
             << original_bytes << " -> " << quantized_bytes << ")\n"
             << flush;
...
void quantize_file(const fs::path& input_dir,
                   ...
                   QuantizationSummary& summary) {
    if (options.u_transpose) {
        quantize_file_u_transpose(input_dir, output_dir, name, options, summary);
        return;
    }
    if (options.q_per_snp_column) {
...
QuantizationSummary quantize_directory(const fs::path& input_dir,
    ...
    if (options.u_transpose && options.q_per_snp_column) {
        throw runtime_error("Choose only one layout option: --u-transpose or --q-per-snp.");
    }
