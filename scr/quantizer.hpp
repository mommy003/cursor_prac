...
struct QuantizationOptions {
    ...
    /**
     * If true, quantize U (m x k) but store row-major by SNP row to match UTC runtime:
     * raw index i*k + j corresponds to U(i,j), with one scale per eigen-column j.
     * Writes .eigen.u*utc.bin (and .eigen.u8eutc.bin with --entropy).
     */
    bool u_transpose = false;
};
